...
// function that allocate a new resource and the size of the pool.
...
func (p *Pool) Release(r io.Closer) {
	...
	case p.resources <- r:
		...
		// If the queue is already at capacity w close the resource.
	default:
...
func (p *Pool) Close() {
	...
	// If the pool is already closed, donot do anything.
	if p.closed {
		return
	}
	...
	// resources. If we donot do this, we will have a deadlock.
	close(p.resources)
